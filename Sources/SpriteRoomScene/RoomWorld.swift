...
/// back upstream.
@MainActor
/// **One project's room: every node in it, and all the state behind them.**
///
/// This was `RoomScene` and was an `SKScene`, which meant the room and the
/// camera were one object and there could therefore be exactly one room. M9
/// Phase 4 needs several at once, one per project, so the two were separated:
/// everything that is *about a room* is here, and `RoomScene` keeps only what
/// is about the **view** (the camera, the viewport, the integer scale) and owns
/// one of these.
///
/// **The extraction moved no behaviour.** Every method below is the one that
/// was here before it, and the seam was chosen so that almost nothing had to
/// change to cross it: the room already built itself into a single container
/// node rather than into the scene, so that node simply became this object's
/// own. The three places that genuinely touched the view are the three that
/// moved out, and one of them (`positionOverflowPlate`) is now handed the
/// camera's frame instead of reading it off a scene it no longer belongs to.
///
/// Proved by `scripts/lint-palette.py`'s scene-agreement check, which compares
/// the room this draws against an independent transcription pixel for pixel,
/// per theme, with an empty defect register.
public final class RoomWorld {

    public let layout: RoomLayout
    public let store: TextureStore
    /// Everything this room draws. `RoomScene` adds it to itself; a second room
    /// would be a second one of these under the same camera.
    ...
    /// **`RoomScene.apply` is the entry point, not this.** It re-frames the
    /// camera afterwards, which this object cannot do and must not pretend to.
    ...
    public func apply(_ intent: SpriteIntent) {
        ...
        case .setScale:
            // **Handled by `RoomScene`, which owns the camera.** [I6] It is
            // intercepted before the batch reaches this object, so this arm is
            // unreachable; it stays as an arm rather than a `default` so that a
            // new intent cannot be swallowed here by accident.
            break
    ...
    /// - Parameter frame: the camera's frame in room coordinates. It used to be
    ///   read off `size` and `camera_.position` directly; the room no longer
    ///   owns either, so the one thing here that is genuinely about the view is
    ///   passed in. The arithmetic is unchanged.
    func positionOverflowPlate(in frame: CGRect) {
    ...
    /// The frame the plate was last clamped into, so `setOverflow` can re-clamp
    /// without the caller having to hand it one. `RoomScene` refreshes it on
    /// every `applyScale`.
    private(set) var lastCameraFrame = CGRect(x: 0, y: 0, width: 960, height: 540)

    func rememberCameraFrame(_ frame: CGRect) { lastCameraFrame = frame }
    ...
    /// The room is driven, never self-driving: `RoomScene.update` forwards the
    /// view's render loop here and the offscreen harness calls it directly with
    /// a simulated clock. One animation engine, two drivers, identical output.
    ...
    // MARK: What the camera needs from the room [I6]
